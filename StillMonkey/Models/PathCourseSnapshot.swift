...
struct PathCourseSnapshot: Codable, Identifiable {
    ...
    /// Clears cached reels/quiz for a lesson so content can be regenerated (reload).
    mutating func removeLessonProgress(for lessonID: String) {
        lessonProgressByID[lessonID] = nil
        updatedAt = .now
    }
